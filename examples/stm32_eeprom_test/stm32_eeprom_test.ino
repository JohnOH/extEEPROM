...
//        if ( (addr &0xFFF) == 0 ) Serial << addr << endl;
        if ( (addr &0xFFF) == 0 ) Serial.println(addr); // << endl;
...
//    Serial << "Write lapse: " << msLapse << " ms" << endl;
   Serial.print("Write lapse: "); Serial.print(msLapse); Serial.println(" ms"); // << endl;
}
...
void eeRead(uint8_t chunk)
{
    ...
    for (uint32_t addr = 0; addr < totalKBytes * 1024; addr += chunk) {
        ...
        for (uint8_t c = 0; c < chunk; c += 4) {
            ...
            if (testVal != val) Serial.print("Error @ addr "); // << addr+c << F(" Expected ") << val << F(" Read ") << testVal << F(" 0x") << _HEX(testVal) << endl;
            ++val;
        }
    }
    uint32_t msLapse = millis() - msStart;
    Serial.print("Last value: "); Serial.println (val); // << " Read lapse: " << msLapse << " ms" << endl;
}
...
{
    Serial.print("EEPROM DUMP 0x"); Serial.print(startAddr, HEX); Serial.print(" 0x"); Serial.print(nBytes); Serial.print(startAddr); Serial.print(' '); Serial.println(nBytes);
    delay(10000);
    ...
    for (uint32_t r = 0; r < nRows; r++) {
        ...
        for ( int c = 0; c < 16; c++ ) {
            ...
            Serial.print(d[c], HEX); Serial.print( c == 7 ? "  " : " " );
        }
        Serial.println();
    }
}
